...
第 6 节的策略是"只导出实际用到的具体实例化"，但**"实际用到了哪些"这个集合怎么算出来**本身就是一个难点，值得单独展开。本附录给出问题根源、三条可行路径，以及推荐的工程做法。

### A.1 为什么"发现"本身就是难点

根子在于：模板实例化是编译期、隐式、且散布在整个调用方代码里的行为，而不是一个能在库定义处一眼看全的清单。最容易想到的 grep 'Container<' 正则法，在真实代码库里会同时漏报和误报，因为 C++ 的类型书写方式花样太多：


别名遮蔽：using IntBox = Container<int>; 之后代码里全写 IntBox，grep 抓不到。typedef、template alias（template<class T> using Vec = Container<T>;）同理。
CTAD / auto 推导：Container c{1, 2, 3}; 源码里根本没写模板参数，类型是推导出来的。
默认模板参数：源码写的是 Container<int>，真正实例化的规范类型（canonical type）可能是 Container<int, std::allocator<int>>。shim 命名和去重必须基于补全默认参数后的规范类型，否则同一个实例化会被算成两个、或两个不同的被当成一个——而正则永远看不到被省略的那部分。
传递实例化：库自己的模板内部实例化出来的类型（Container<Node> 里又用到 std::vector<Node>），调用方源码里完全不出现。
宏 / 条件编译：#ifdef _WIN32 分支下才出现的实例化，取决于在哪个平台、哪套宏下看。


所以"发现"不能靠文本匹配，得靠理解类型系统的工具。有两个层面的权威来源：源码级（编译器前端的 AST）和产物级（编译产物里的符号 / 调试信息）。

### A.2 路径一：Clang AST —— 源码级的权威答案

用 Clang 的语义分析结果去数实例化，这是最正的方法。关键 AST 节点是 ClassTemplateSpecializationDecl（类模板的每个具体特化 / 实例化），函数模板则是 functionDecl(isTemplateInstantiation())。它已经帮你解决了别名、CTAD、默认参数——因为你拿到的是补全后的规范类型。

快速探查用 clang-query：

clang-query> match classTemplateSpecializationDecl(hasName("::myproj::Container"))

工程化则写一个 LibTooling / RecursiveASTVisitor，在 VisitClassTemplateSpecializationDecl 里取 getTemplateArgs() 拿实参、getCanonicalType() 规范化、getSpecializationKind() 区分隐式实例化还是显式实例化，把结果拍平成去重清单喂给 Manifest。

这条路有两个硬约束必须讲清楚：


必须喂真实的编译参数。用 CMake CMAKE_EXPORT_COMPILE_COMMANDS=ON 生成 compile_commands.json，让工具用和正式构建完全一致的 -I、-D、-std、平台宏去解析，否则条件编译分支、宏展开都会错位。
必须覆盖全部触发实例化的 TU，而不只是库头文件。隐式实例化只在"触发它的那个翻译单元"的 AST 里出现。只解析库本身，看到的是零；必须把所有使用该库的 TU（包括这次要生成 binding 的调用点）都跑一遍再求并集。反过来，如果某个实例化只在分析范围之外的代码（比如别的下游项目）里被触发，这条路就看不到——这正是需要路径二兜底的原因。


### A.3 路径二：编译产物的 DWARF / 符号表 —— "实际发射了什么"的地面真相

AST 告诉你"源码语义上会实例化什么"，编译产物告诉你"编译器实际吐出了什么符号 / 类型"，两者互补。

拿最终的 .a/.so，用 nm -C 看去混淆后的符号：成员函数的 mangled 名字里编码了具体模板实参，_ZN9ContainerIiE3getEv 解出来就是 Container<int>::get()。或者更全面地，带 -g 编译后用 llvm-dwarfdump 读调试信息里的 DW_TAG_class_type，其 DW_AT_name 直接就是 Container<int> 这样的完整名字，还带 DW_TAG_template_type_parameter 子节点标出每个模板参数。

这条路的注意点：高优化等级（-O2）可能把某些实例化的函数全部内联掉、不发射独立符号，导致漏看；所以发现用的这一遍应该用 -O0 -g（必要时 -fno-inline）专门构建一次。它的价值在于：这是"编译器实实在在处理过的实例化"的完整快照，能兜住路径一因分析范围不全而漏掉的东西，也能作为交叉验证——两条路的结果对不上，就说明有分析盲区。

### A.4 推荐做法：把"发现"反转成"约束 + 差异检查"

纯发现有个根本缺陷：它是开集问题，对代码演进天然脆弱。今天扫出 5 个实例化生成了 shim，明天有人在业务代码里新写了一个 Container<Foo>，你的 shim 会静默地不覆盖它，直到运行期才炸。这和整本手册"static_assert 防漂移、ABI 版本常量 fail-fast"的哲学是相悖的。

更稳的做法是把开集变成闭集，分三步：


在 Manifest 里维护一份显式的"受支持实例化清单"，作为唯一事实来源。
在一个 shim TU 里对清单里的每一项做显式实例化：template class myproj::Container<int>;。这一步一石二鸟——既强制编译器发射出稳定符号（保证 bindgen/链接拿得到），又把这套实例化的 ABI 钉死在你控制的地方，而不是散落在各调用点由隐式实例化决定。
用路径一的 Clang AST pass 做 CI 差异门禁：扫全代码库，凡是目标模板的 ClassTemplateSpecializationDecl 出现了、但不在受支持清单里的，就报错 / 告警。这样"用到了一个没导出的新实例化"从静默漏洞变成一次响亮的构建失败。


于是初次接入时，路径一 / 二用来自动生成清单初稿（把人从手写枚举里解放出来）；之后清单进入版本控制，发现工具退居为守门员而非唯一生产者。这也顺带回答了第 6 节里"模板参数种类很多要不要收窄成 void* + 类型标签"的决策点：当 CI 告警频繁跳出、清单膨胀到难维护时，就是该收窄的信号。

cpp// shim 的一个专用 TU：显式实例化 + 钉死 ABI，符号稳定可链接
template class myproj::Container<int>;
template class myproj::Container<std::string>;

// 对应导出的具名 shim 函数（第 6 节风格）
typedef struct ContainerIntOpaque* ContainerIntHandle;
bool container_int_get(ContainerIntHandle h, size_t idx, int32_t* out);

**小结**：发现模板实例化 = Clang AST（源码级权威、解决别名/CTAD/默认参数）+ DWARF/符号（产物级兜底与交叉验证）+ 显式清单（把开集收敛成闭集、CI 差异门禁防演进漂移）。三者组合，而不是依赖任何单一手段，更不是依赖文本匹配。
